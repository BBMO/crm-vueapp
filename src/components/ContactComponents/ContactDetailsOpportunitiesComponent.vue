<template>
  <q-card class="full-width">
    <div class="q-pa-md flex items-center gap-sm">
      <q-icon size="sm" name="mdi-file-sign" color="primary" />
      <h6 class="text-subtitle1 q-ma-none text-weight-medium">{{ $t('contact.opportunities') }}</h6>
    </div>
    <q-table
        row-key="name"
...
const getOpportunities = async () => {
  ...
  const { data } = await ContactsService.getContactOpportunities(contactId.value);
